...
class ParallelArrayDictionaryTest
{
	...
	void testParallelArrayDictionary()
	{
		...
		for (int i = 0; i < keys.size(); i++) {
			assertEquals((values.get(i)), pad.get(i));
		}
	}
	...
	void testGet()
	{
		...
		for (Integer key : keys) {
			assertEquals(values.get(key), getTest.get(key));
		}
	}
	...
	void testPut()
	{
		...
		for (int i = 0; i < keys.size(); i++) {
			assertEquals(values.get(i), putTest.put(keys.get(i),values2.get(i)));
		}
	...
	void testRemove()
	{
		...
		List<Integer> keys = Arrays.asList(0,1,2,3,4,5,-1);
		...
		for (int i = removeTest.size() - 1; i >= 0; i--) {
			assertEquals(values.get(keys.indexOf(keys.get(i))), removeTest.remove(keys.get(i)));
		}
		...
		for (int i = 0; i < keys2.size(); i++) {
			removeTest2.put(keys2.get(i), values2.get(i));
		}
	...
	void testPutAll()
	{
		...
		List<Integer> keys = Arrays.asList(0,1,2,3,-1);
		List<Integer> values = Arrays.asList(4,6,11,9,11);
		...
		for (int i = 0; i < keys.size(); i++) {
			assertEquals((values.get(keys.indexOf(keys.get(i)))), putAll2.get(keys.get(i)));
		}
	}
	...
	void testClear()
	{
		...
		//verifies existence
		for (int i = 0; i < keys.size(); i++) {
			assertEquals((values.get(i)), clearTest.get(i));
		}
